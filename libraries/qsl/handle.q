...
.hnd.hopen:{[servers;tmout;flag]
  ...
  `.hnd.status upsert ([server:key servers];timeout:tmout;state:`registered;connstr:value servers;handle:.hnd.p.hopenLazy @/: key servers;ashandle:.hnd.p.asHopenLazy @/: key servers;rawHandle:0Ni;topen:0Np;tclose:0Np;tlost:0Np;reconn:0;msg:(count servers)#enlist"");
...
.hnd.hclose:{[servers]
  if[1~count .hnd.status;:0]; // .hnd.hopen has not beed called yet
  ...
  hclose each openh: exec rawHandle from .hnd.status where server in openservs, not rawHandle=0;
  ...
  .hnd.status[enlist each servers;`rawHandle]:0i;
...
.hnd.p.hopenLazy:{[s;q]
  ...
  .hnd.status[s;`handle`ashandle`rawHandle`topen`state]:(sh;neg h;h;.z.p;`open);
  .hnd.po[.hnd.p.cb s] s; // run callbacks
  :h q
  };

/F/ A wrapper for a raw handle that is used for remote connections
/-/ A projection .hnd.p.rh[s;h] of this function is put in .hnd.status in the handle column
/P/ s:SYMBOL - server name
/P/ h:LONG - the raw handle
.hnd.p.rh:{[s;h;q] :.pe.at[h;q;.hnd.p.onRemoteFailure[s;q]] };


/F/ Checks if the connection string is to the localhos
/P/ connstr:SYMBOL - the connection strings
/R/ :BOOL - 1b if this is a localhost connestion, 0b otherwise
.hnd.p.isLocahost:{[connstr] (":" vs string connstr)[1] in ("";"localhost";"127.0.0.1") };

/F/ Function run when .hnd.h fails when running query on a process running on a remote host.
/-/ This may happen when the process got restarted which invalidated the handle, but the reconnect
/-/ was not executed on port close which happens sometimes on remote connections.
/-/ The function tries to connect again and if it fails the function logs error and starts the reconnection timer.
.hnd.p.onRemoteFailure:{[s;q;e]
  if[not e~": Bad file descriptor";'e];
  .hnd.refresh[s];
  if[`open~.hnd.status[`remoteProcess;`state];:.hnd.h[s] q];
  '"Remote process ",(.Q.s1 s)," unavailable, reconnect is running";
  };
...
.hnd.p.hopenEager:{[s]
  ...
  if[not h~0N;
    // use handle if local connection but wrapped in connection check if remote
    sh:$[.hnd.p.isLocahost connstr;h;.hnd.p.rh[s;h]];
    .hnd.status[s;`handle`ashandle`rawHandle`topen`state]:(sh;neg h;h;.z.p;`open);
    .hnd.po[.hnd.p.cb s] s; // run callbacks
    ];
...
/F/ Error handler for opening in the eager mode.
/P/ s:SYMBOL - server name
/P/ e:SYMBOL - the error (signal)
.hnd.p.errEager:{[s;e]
  sig:"can't open connection to ",string[s],", error: ",e;
  .hnd.status[s;`handle`rawHandle`topen`state`msg]:(.hnd.p.hopenLazy[s];0Ni;0Np;`failed;sig);
  .log.warn[`handle] sig;
...
.hnd.p.pc:{[h]
  ...
  ser:exec server from .hnd.status where rawHandle ~\: h;
  if[not 1~count ser;:(::)]; // not our handle
  .log.warn[`handle] "Server ",string[first ser], " with handle: ", string[h], " has been disconnected";
  s:first ser;
  // update status
  .hnd.status[s;`handle`ashandle`rawHandle`state`tlost`msg]:(.hnd.p.hopenLazy[s];.hnd.p.asHopenLazy[s];0Ni;`lost;.sl.zp[];"remote server closed unexpectedly");
...
.hnd.p.rec:{[s;t]
  ...
  if[not h~0N;
    ...
    sh:$[.hnd.p.isLocahost connstr;h;.hnd.p.rh[s;h]];
    .hnd.status[s;`handle`ashandle`rawHandle`topen`state]:(sh;neg h;h;.sl.zp[];`open);
    .hnd.po[.hnd.p.cb s] s; // run callbacks
    ];
  };
...
/P/ s:SYMBOL - server name
.hnd.p.hclose:{[s]
  if[`open~.hnd.status[s;`state];hclose .hnd.status[s;`rawHandle]];
  .tmr.stop[`$".hnd.rec.",(.hnd.p.remdot s)];
  };
...
.hnd.p.setGlobals:{[]
  ...
  /E/ .hnd.status
  .hnd.status:([server:1#`]timeout:1#0N; state:1#`; connstr:1#`; handle:1#();ashandle:1#(); rawHandle:1#0Ni; topen:1#0Np; tclose: 1#0Np; tlost:1#0Np; reconn:1#0N; msg:1#(); remoteHnd:1#0N);
